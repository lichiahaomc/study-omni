...
前端为纯静态资源(19 个文件,无构建 —— 直接加载源码,没有打包步骤),
后端用 Cloudflare Pages Functions 做代理层
...
public/              静态资源,即 wrangler.toml 里的 pages_build_output_dir
  index.html         页面结构(1180 行)+ 首屏预初始化脚本(内联)
  css/  7 个         样式,按 01–07 顺序加载
  js/  11 个         脚本,按 01–11 顺序加载
functions/           Cloudflare Functions(必须在项目根,不能放进 public)
  ...
  _lib/              共享模块(下划线开头,不映射成路由)
    providers.js     模型接入(DeepSeek 单家)+ 档位表 + token 预算 + 端点管束
    prompts.js       学科提示词生成器(61 门学科,三层组合)
    ratelimit.js     限流 + 同源校验
scripts/
  split.mjs          把 index.html 切成 css/ js/(带三道自检,见下)
  toc.mjs            生成 index.html 里的源码索引
tests/               离线测试,`npm test` 全跑
  theme.test.mjs     配色算法 + 玻璃通透度 + 弹窗层叠结构 + 目录结构
...
早期是零依赖零构建的**单文件**,涨到 8000 行后找东西全靠翻。
现在按"一个关注点一个文件"切开,仍然**没有任何构建步骤** ——
`<link>` 和 `<script src>` 直接加载源文件,改完刷新就是最新的。

- **顺序是硬约束**:CSS 级联和 JS 执行都吃顺序,文件名前缀 `01–07` / `01–11`
  就是用来对齐 `index.html` 里的加载顺序的,**不要调**。
  测试里有一条断言专门盯着"引用顺序 == 文件名顺序"。
- **切分是脚本做的,不是手抄的**:`scripts/split.mjs` 按分节横幅切,
  并且落盘前跑三道自检 —— 切出的片段能否原样拼回、原文每一行是否都被认领、
  落盘后读回是否一致。任何一道不过就拒绝写文件。
- **首屏预初始化脚本仍然内联**在 `<head>` 里:它要在首次绘制前把主题铺好,
  拆成外部文件会多一次网络往返,深色用户会看到白屏闪一下。
...
`public/index.html` 的 `<head>` 里带一份**生成出来的源码索引**:按加载顺序列出
19 个文件、每个多少行、以及每个文件内部的分节和行号 —— 打开入口文件就能知道
"某个东西该去哪个文件找"。

```bash
npm run toc          # 改了分节横幅之后重新生成索引
node scripts/split.mjs --check   # 只跑切分自检,不写文件
```

`npm test` 的第一件事就是校验这份索引与实际文件一致,并逐个核验行号是否真的指向
一条横幅 —— 不一致直接失败。**手写索引一定会过期**,别再改回手写。
