...
—— **默认的模型密钥只存在云端 Secret 里,浏览器与前端代码都不持有**。
(也支持 BYOK:用户在设置里填自己的密钥,详见「自定义 API」一节。)
...
设置 → 「自定义 API」里可以填自己的密钥,以及一个可选的 API 地址。
填了就优先用你的,点「恢复默认」清掉,回到服务端配置。

三条设计取舍:

- **密钥不进设置状态**。设置面板有「导出设置」,一旦密钥进了 state 就会被导出成
  JSON 文件到处传。所以它单独存两个 localStorage 键,和主题/字号那批彻底分开 ——
  导出设置里**永远不会**出现密钥。
- **地址只接受 https 公共域名**。放开"客户端可指定端点"这件事本身有风险:
  Worker 会变成 SSRF 跳板,被拿去打内网或云元数据地址。
  所以 `localhost` / `*.local` / `*.internal` / 回环 / 私网段(`10.` `172.16-31.` `192.168.`)
  / 链路本地(`169.254.`)/ CGNAT / IPv6 一律拒绝,带账号密码或查询串的也拒绝。
  前端只做形态检查给即时反馈,**真正的管束在服务端**。
- **校验不过就静默回落**,不报错 —— 否则这个接口本身会变成"帮你探测地址合不合法"的工具。

只填域名(如 `https://api.deepseek.com`)时会自动补上 `/chat/completions`,
不用自己拼完整路径。

> 密钥每次请求通过请求头带上,服务端用完即弃:不落库、不写日志、不回传。
